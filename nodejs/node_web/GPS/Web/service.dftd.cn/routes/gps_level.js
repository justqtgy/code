...
/* GET gps_level home page. */
router.get('/', function(req, res, next) {
    var start_date = new Date().add({ days: -10 }).toFormat('YYYY-MM-DD'),
        end_date = new Date().toFormat('YYYY-MM-DD');
    res.render('gps_level', { start_date: start_date, end_date: end_date });
});

var get_count = function(req, res, next) {
    var args = req.body;
    gps_level.get_count(args, function(err, result) {
        if (err) {
            log.error('Error = ', err);
            next();
            return;
...
router.post('/list', [get_count], function(req, res, next) {
    var args = req.body;
    gps_level.get_list(args, function(err, rows) {
        if (err) {
            log.error('Error = ', err);
            return;
        }
