...
dao.add_gps_data = function(args, callback) {
    ...
    sql = util.format(sql, args.gps_id,args.sn,args.num,args.one_zl,args.rl,args.one_c1,args.one_c2,args.two_zl, args.two_rl, args.two_c1, args.two_c2,
...
dao.add_gps_last = function(args) {
    var sql = `
        insert into gps_last(gps_id, sn, num, one_zl, one_rl, one_c1, one_c2, two_zl, two_rl, two_c1, two_c2,
             power, lng, lat, dist_id, distance, gps_time, alarm_info, add_time)
            values('%s', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
             %s, %s, %s, %s, %s, '%s', '%s', %s, %s, '%s', %s, GETDATE())
        ;SELECT @@IDENTITY as ID;`;
    sql = util.format(sql, args.gps_id,args.sn,args.num,args.one_zl,args.rl,args.one_c1,args.one_c2,args.two_zl, args.two_rl, args.two_c1, args.two_c2,
...
dao.add_gps_last = function(args) {
    var sql = `
        ...
        set one_zl = %s, one_rl=%s, one_c1=%s, one_c2=%s, 
                ...
                alarm1 = %s, alarm2 = %s, update_time=GETDATE()
        where gps_id='%s'
    `;
    sql = util.format(sql, args.one_zl,args.rl,args.one_c1,args.one_c2,args.two_zl, args.two_rl, args.two_c1, args.two_c2,
            args.power, args.lng, args.lat, args.dist_id, args.distance, args.gps_time, args.gps_id);
    db.execSQL(sql, function(err, result){
        if (err) {
            logger.error('Error = ', err);
        }
...
dao.add_gps_alarm = function(args) {
    ...
    sql = util.format(sql, args.gps_id, args.one_zl,args.one_rl,args.one_c1,args.one_c2,args.two_zl, args.two_rl, args.two_c1, args.two_c2,
            args.lng, args.lat, args.gps_time, args.alarm_type);
    db.execSQL(sql, function(err, result) {
        if (err) {
            logger.error('Error = ', err);
        }
...
dao.add_gps_capacity = function(args, callback) {
    var sql = `
