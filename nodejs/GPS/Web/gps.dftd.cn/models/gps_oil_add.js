...
gps_oil_add.get_count = function(params, callback) {
    var sql = " select count(*) as total from gserver_packet.dbo.gps_oil_add where addtime >='%s' and addtime<dateadd(day, 1, '%s')";
...
gps_oil_add.get_list = function(params, callback) {
    ...
    var sql = ";with t as (select row_number() over(order by addtime desc) as rid, *, convert(varchar(10), AddTime, 120) as CreateDate " +
        "   from gserver_packet.dbo.gps_oil_add  " +
