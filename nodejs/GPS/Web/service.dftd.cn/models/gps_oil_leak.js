...
gps_oil_leak.get_count = function(params, callback) {
    console.log(params);
    var sql = " select count(*) as total from gps_oil_leak where addtime >='%s' and addtime<dateadd(day, 1, '%s') and vehicleid in (%s) and GPSStatus=1";
...
gps_oil_leak.get_list = function(params, callback) {
    ...
    var sql = ";with t as (select *, row_number() over(order by addtime desc) as rid  from gps_oil_leak " +
