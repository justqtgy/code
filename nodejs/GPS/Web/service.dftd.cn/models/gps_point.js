...
gps_point.get_count = function(params, callback) {
    console.log(params);
    var sql = " select count(*) as total from gps_point where addtime1 >='%s' and addtime1<dateadd(day, 1, '%s') and vehicleid in (%s)";
...
gps_point.get_list = function(params, callback) {
    ...
    var sql = ";with t as (select *, row_number() over(order by id desc) as rid  from gps_point  where addtime1 >='%s' and addtime1<dateadd(day, 1, '%s')  and vehicleid in (%s)) " +
