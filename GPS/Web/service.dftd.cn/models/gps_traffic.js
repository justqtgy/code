...
gps_traffic.get_count = function(params, callback) {
    console.log(params);
    var sql = " select count(*) as total from gps_traffic where addtime >='%s' and addtime<dateadd(day, 1, '%s')";
...
gps_traffic.get_list = function(params, callback) {
    ...
    var sql = ";with t as (select *, row_number() over(order by addtime desc) as rid  from gps_traffic  where addtime >='%s' and addtime<dateadd(day, 1, '%s'))  " +
