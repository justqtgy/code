...
gps_traffic.get_count = function(callback) {
    db.execSQL('select count(*) as total from gps_traffic', function(err, result) {
        if (err) {
            console.log(err);
            throw err
        }
...
gps_traffic.get_list = function(params, callback) {
    ...
    var sql = ";with t as (select *, row_number() over(order by id desc) as rid  from gps_traffic)  " +
